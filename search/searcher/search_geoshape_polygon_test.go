...
func TestGeoJsonPolygonIntersectsQuery(t *testing.T) {
	...
	for n, test := range tests {
		...
		if !reflect.DeepEqual(got, test.want) {
			t.Errorf(
				"test %d, expected %v, got %v for polygon: %+v", n,
...
func TestGeoJsonPolygonContainsQuery(t *testing.T) {
	...
	for n, test := range tests {
		...
		if !reflect.DeepEqual(got, test.want) {
			t.Errorf(
				"test %d, expected %v, got %v for polygon: %+v",
...
func runGeoShapePolygonQueryWithRelation(
	relation string, i index.IndexReader,
...
func setupGeoJsonShapesIndexForPolygonQuery(t *testing.T) index.Index {
	...
	doc.AddField(
		document.NewGeoCircleFieldWithIndexingOptions(
			"geometry",
...
func runGeoShapeMultiPolygonQueryWithRelation(
	relation string,
	i index.IndexReader,
	points [][][][]float64, field string,
) ([]string, error) {
		...
		{
			{
				...
				{-121.48638725280762, 38.53213710006686}, {-121.49140834808348, 38.5320028163074},
			},
		}, // polygon1
		{
			{
